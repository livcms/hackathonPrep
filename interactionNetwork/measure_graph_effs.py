...
data_dir = '/home/lhv14/validationNtuple/ntuple_PU200/'
files = [f for f in listdir(data_dir)]
#files = [f.split('.')[0] for f in files if "hits" in f]
evt_ids = [f.split('ntuple_PU200_event')[1].split('.')[0] for f in files]
...
graph_dir = '/home/lhv14/cmsgnn/output/PU200/0.5GeV/'
...
for i, evtid in enumerate(evt_ids):
    ...
    if not os.path.isfile(graph_path):
        print("could not find ", graph_path)
        continue
...
#    hits, cells, particles, truth = load_event(os.path.join(data_dir, evtid))
    data = pd.read_hdf(data_dir + 'ntuple_PU200_event' + evtid +'.h5')
    hits_by_loc = data.groupby(['volume_id', 'layer_id'])
   ...
   # n_particles_before = particles.particle_id.unique().shape[0]
    n_particles_before = len(data.sim_id.unique()) 
    #pt = np.sqrt(particles.px**2 + particles.py**2)
    particles = data[data['sim_pt'] > pt_cut]
    ...
    #edited from .loc etc 
    #hits =  data.droplevel(level=0).groupby(['sim_id', 'layer_id'], as_index=False).r.idxmin()

   # print(data.droplevel(level=0).groupby(['sim_id', 'layer_id'], as_index=False).r.idxmin())

    
    #particle_ids = hits.sim_id.unique()
    #n_particles_after = particle_ids.shape[0]

    #print(hits.droplevel(level=0))
    #particle_list = hits.droplevel(level=0).groupby('sim_id')
    
    truth_edges_per_particle = {}
    truth_edges =  0
    for particles in data['sim_id'].unique():
        particle = data[data['sim_id']==particles]
        #print(particle)
        #if (p_id == 0): continue
        ...
        if (n_layers_hit > 1): 
            layers = np.array(particle.layers)
        ...
        print(particle)
        truth_edges_per_particle[particle['sim_id']] = n_layers_hit-1
